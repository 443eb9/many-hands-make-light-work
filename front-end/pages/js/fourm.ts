import { Api, provinces, siteAvatarUrl } from "./constants.js";
import { CUR_USER_ID, EXPECTED_METHOD, EXPECTED_METHOD_STR, PROVINCE_EXCLUDED, PROVINCE_EXCLUDED_STR } from "./cookies.js";
...
async function fetchPosts() {
    let data =
        `{"isDetailed":false,"excludedProvince":[` + PROVINCE_EXCLUDED_STR +
        `],"expectedMethod":[` + EXPECTED_METHOD_STR + `]}`;
    let posts: Post[] = await request(Api.GetPost, data);
    for (let i = 0; i < posts.length; ++i) {
        postContainer.insertAdjacentHTML("beforeend", `
        <div class="post">
            <div class="info">
                <div class="user-info">
                    <img src="`+ siteAvatarUrl + posts[i].author.id + `.jpg" class="avatar"></img>
...
async function fetchMyInfo() {
    let me: User = await request(Api.GetUser, CUR_USER_ID);
